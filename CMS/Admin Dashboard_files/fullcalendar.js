...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (seg.isStart) {
			...
			if (timeText) {
				//timeHtml = '<span class="fc-time">' + 'Time:- '+ htmlEscape(timeText) + '</span><br/>';
				timeHtml = '';
			}
		}

		titleHtml =
			'<span class="fc-title"><strong>' +
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($('<h2 style="font-size: 20px;font-weight: bold;">&nbsp;</h2>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
